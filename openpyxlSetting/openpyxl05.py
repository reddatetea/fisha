...
#创立六个字典，分别是期初数、入库数量、采购入库，出库数量，生产领料，结余数
qichu_dic = {}
ruku_dic = {}
caigou_dic ={}
banchengpin_dic = {}
...
for index,row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10]  in qichu_dic:
            qichu_dic[row[10]] = qichu + row[3]
        else:
            qichu_dic[row[10]] = row[3]
...
for index, row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10] in ruku_dic:
            ruku_dic[row[10]] = ruku + row[4]
        else:
            ruku_dic[row[10]] = row[4]
...
for index, row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10] in caigou_dic:
            caigou_dic[row[10]] = caigou + row[5]
        else:
            caigou_dic[row[10]] = row[5]
...
for index, row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10] in banchengpin_dic:
            banchengpin_dic[row[10]] = banchengpin + row[6]
        else:
            banchengpin_dic[row[10]] = row[6]
...
for index, row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10] in chuku_dic:
            chuku_dic[row[10]] = chuku + row[7]
        else:
            chuku_dic[row[10]] = row[7]
...
for index, row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10] in lingliao_dic:
            lingliao_dic[row[10]] = lingliao + row[8]
        else:
            lingliao_dic[row[10]] = row[8]
...
for index, row in enumerate(sheet.values):
    if index == 0:
        ...
        # 将上面计算出的实际行数，运行到下面的程序！不这样就会报错！
    if index < mrows:
        if row[10] in qimo_dic:
            qimo_dic[row[10]] = qimo + row[9]
        else:
            qimo_dic[row[10]] = row[9]

print(qichu_dic)
print(ruku_dic)
print(caigou_dic)
print(banchengpin_dic)
print(chuku_dic)
print(lingliao_dic)
print(qimo_dic)
