...
#将xls文件转化为xlsx
fname = "F:\\00nutstore\\fishc\\双佳数量.xls"
...
for index,row in enumerate(sheet.values):
    if index == 0:
        ...
        if index < mrows:
            ...
            if row[5] == '借':
                qichushuliang = float(row[7])
            else:
                qichushuliang = -1*float(row[7])
    ...
    if index == 0 :
        ...
        if index <= mrows:
            ...
            if row[17] in banchengpin_dic :
                banchengpion = banchengpin_dic[row[17]]
            else :
                banchengpin = 0
