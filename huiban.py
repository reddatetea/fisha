...
def huibanHedui(im_gongyingshang,fname,ws_name):
    ...
    for row in range(1,maxrow+1):
        if row == 1 :
            ...
            hou, chang, kuan = huibanjisuan01.huibanchicun(pinming)
            hetong_danjia = huibanjisuan01.jisuanDanjia(im_gongyingshang,hou,chang,kuan)
...
def main():
    ...
    for im_gongyingshang in im_gongyingshangs:
        ...
        fname1, ws_name1 = tiqushuju.tiquShuju(fname, im_gongyingshang, im_qijians)
        fname1, sheetnames = huibanHedui(im_gongyingshang,fname1, ws_name1)
        letters = 'FTV'
        addSum.sumheji(fname1, sheetnames, letters)
        path, filename = os.path.split(fname1)
        choice = easygui.ccbox(msg='是否打开要查询的文件"%s"' % filename, title='{}灰板价格核对'.format(im_gongyingshang), choices=('yes', 'no'))
        if choice == True:
            os.startfile(fname1)
        else:
            pass
