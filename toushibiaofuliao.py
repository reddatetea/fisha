...
def choiceSheet():
    ...
    hangziduan ='cwName'
    jisuanfangsi = 'sum'
    saixuanliezhi = input('请输入期间（如2022-06)\n')
    wb.close()
    return fname, ws_name, zhiziduan,hangziduan, jisuanfangsi,saixuanliezhi

def toushiBiao(fname, ws_name, zhiziduan,hangziduan,jisuanfangsi,saixuanliezhi):
    ...
    gongyingshang = easygui.choicebox(msg,title='供应商',choices=gongyingshangs)
    print(gongyingshang)
    values = ws.range('A1').expand('table').options(pd.DataFrame).value
...
def main():
    ...
    gongyingshang =toushiBiao(fname, ws_name, zhiziduan,hangziduan,jisuanfangsi,saixuanliezhi)
