...
def gongyingShangs(fname1):
    ...
    df1 = pd.read_excel(fname1, dtype={'供应商编号': str, '科目编码': str})  # 低版本pandas适用
    df1.rename(columns={'供应商名称': '供应商',
                        ...
                        '贷方金额': '购入'},
               inplace=True)
    dic = dict(zip(df1['供应商'], df1['科目代码']))
    df8 = pd.DataFrame.from_dict(dic, orient='index')
    df8 = df8.reset_index()
    df8.columns = ['供应商', '科目代码']
    # df8 = df1[['供应商', '科目代码']]
    df1.pop('科目代码')

    df1['期初余额'] = df1['期初余额'] * (df1['方向4'].agg(lambda x: -1 if x == '借' else 1))
    df1['期末余额金额'] = df1['期末余额金额'] * (df1['方向20'].agg(lambda x: -1 if x == '借' else 1))
    df1.pop('科目名称')
    df1.pop('方向4')
    df1.pop('方向20')
    ...
    df_zangu = df1[df1['科目编码'] == '2272']
    df_zangu = df_zangu[['供应商', '期末余额金额']]
    df_zangu = df_zangu[df_zangu['期末余额金额'] < 0]
    df_zangu.columns = ['供应商', 'koukuan2']
    df1 = df1[df1['科目编码'] == '2271']
    df1.pop('科目编码')
    df1.pop('期末余额金额')
    return df1, df8, gongyingshangs, df_zangu
...
def shujuLianjieToushi(df1, df2, df_zangu):
    ...
    df3['付款'] = df3['付款'].fillna(0)
    df3['购入'] = df3['购入'].fillna(0)
    df3['期初余额'] = df3['期初余额'].fillna(0)
    df3['减扣款'] = df3['减扣款'].fillna(0)
    df3['koukuan2'] = df3['koukuan2'].fillna(0)
    df3['koukuan2'] = round(df3['koukuan2']*1.13,2)
...
def addBeizhu(fname, ws_name):
    ...
    with pd.ExcelWriter(fname, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        df.to_excel(writer, sheet_name=ws_name, startrow=shuju_row + 2, header=False, index=False)
...
def main():
    path = r'F:\a00nutstore\006\zw\yingfu\2025yingfu'
    os.chdir(path)
    msg = '请点选本月临时应付账款路径和文件名'
    print(msg)
    fname1 = easygui.fileopenbox(msg)
    df1, df8, gongyingshangs, df_zangu = gongyingShangs(fname1)
    ...
    # ws_name = ws_zangu
    ws_name = easygui.enterbox('请输入本月应付账款名字“0731”')
