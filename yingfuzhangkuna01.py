...
#写入应付账款数据
def yingfuZhangkuan(fname2,gongyingshangs,yingfuzhangkuandic,koukuan,zangudic):
    dirname,filename = os.path.split(fname2)
    os.chdir(dirname)
    wb = openpyxl.load_workbook(fname2)
...
#写入应付账款抬头
    taitou = ('科目代码', '供应商', '期初余额', '付款', '购入', '期末余额', '期初0', '购入0', '累计未认证发票', '本月新增未认证', '扣款',  '暂估有票','暂估无票')
    ws.append(taitou)
...
def main():
    ...
    yingfuZhangkuan(fname,gongyingshangs,yingfuzhangkuandic,koukuan,zangudic)
