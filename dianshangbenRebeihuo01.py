...
# fname = r"F:\a00nutstore\008\zw08\电商\财务部7.18订单.xlsx"
qian, hou = os.path.splitext(fname)
newfname = qian + "-本数" + hou
sheet_name = 0
skiprows = 0
...
bao = gp['包数'].sum()
ben = gp['本数'].sum()
beihuo = pd.concat([bao, ben], axis=1)
beihuo.columns = ['包数', '本数']
beihuo.loc['小计'] = [beihuo['包数'].sum(), beihuo['本数'].sum()]
