...
class SendEmail(object):
    def __init__(self, username, passwd, recv, title, content,
                 file=None, ssl=False,
                 email_host='smtp.sina.com', port=25, ssl_port=465):
...
if __name__ == '__main__':
    file_path = r'd:\searchfile'
    dqrq = datetime.date.today().strftime('%Y%m%d')
    msg = getMessage()
    print(msg)
    file_name = toTXT(dqrq, msg)
    fname = os.path.join(file_path,file_name)
    m = SendEmail(
        username='reddatetea@sina.com',
        passwd='f651372e1972f799',
        recv='Reddatetea01@kindle.cn',
        title='Convert',
        content='python send to kindle',
        file=fname,
        ssl=True,
    )
    m.send_email()
