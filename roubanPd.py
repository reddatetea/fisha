...
def Dangyue(start_riqi,end_riqi):
    ...
    for j in ['单张面积','合同单价', '合同金额', '多计', '多计金额']:
        df[j] = 0.0                             #0.0是flaost64,0是ind64
    df [ '备注'] = None

    df = df.assign(单张面积=df['品名'].map(lambda x: roubanjisuanPd.roubanchicun(x)[0]))
    df['类别'] = df['品名'].map(lambda x: roubanjisuanPd.roubanchicun(x)[-1])
    df = df.reset_index()
