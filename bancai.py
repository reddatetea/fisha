'''
版材核对,由于爱克发的送货日期和入库日期相差较大，连接方式改为按送货日期
'''
...
def Dangyue(start_riqi,end_riqi):
    ...
    for j in ['单张面积','合同单价', '合同金额', '多计', '多计金额','面积','免费药水']:
        df[j] = 0.0                             #0.0是flaost64,0是ind64
    df [ '备注'] = None
    df = df.assign(单张面积=df['品名'].map(lambda x: bancaijisuan.chicun(x)))
    df = df.reset_index()
    return fname,jianchen,df

def Hetong(end_riqi):
    fname = r'F:\a00nutstore\006\zw\price\版材合同价格.xlsx'
    sheet_name = '合同单价'
...
def dfMerge(df1,df2):
    df2 = df2.reset_index()
    df8 = pd.merge(df1, df2, left_on=['送货日期', '供货单位'], right_on=['送货日期', '供货单位'], how='left')  #连接！
    return df8
...
def huizhong(jianchen,fname,df8):
    ...
    with pd.ExcelWriter(fname,datetime_format='yyyy-m-d')  as writer:
        ...
        for gys, values in grouped:
            ...
            values1.at['合计', '免费药水'] = sum(values['免费药水'])
            values1.to_excel(writer, sheet_name='{}'.format(gys), index=False)

def main():
    start_riqi = pd.Timestamp(easygui.enterbox('请输入入库起始日期期间：格式为"2021-11-4"'))
    end_riqi = pd.Timestamp(easygui.enterbox('请输入入库结束日期期间：格式为"2021-12-4"'))
