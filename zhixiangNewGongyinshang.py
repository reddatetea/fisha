...
def zhixiangGongyingshang():
    fname = r'F:\a00nutstore\006\zw\price\纸箱供应商.xlsx'
    wb = openpyxl.load_workbook(fname)
    ws = wb.active
    maxrows = ws.max_row
    zhixiang_gongyingshang_dic = {}
    for row in range(2, maxrows + 1):
        quchen = ws.cell(row, 1).value
        jianchen = ws.cell(row, 2).value
        zhixiang_gongyingshang_dic[quchen] = jianchen
    wb.save(fname)
    return zhixiang_gongyingshang_dic

def main():
    fname = r'F:\a00nutstore\006\zw\price\纸箱供应商.xlsx'
    gongyingshang_dic = zhixiangGongyingshang()
    wb = openpyxl.load_workbook(fname)
    ws = wb.active
    jieshuchaxun = True
    dic = {}
