...
def roubanHedui(fname1,ws_name,im_gongyingshang):
...
def main():
    ...
    gongyingshangs = ['信华','博质','辉盈']
    msg = '请点选供应商'
    im_gongyingshangs = easygui.multchoicebox(msg,title='',choices = gongyingshangs)
    for im_gongyingshang in im_gongyingshangs:
        ...
        yinchanglie.yinchangLie(fname1, ws_name, lies='CIJKLMNOPQR')
        path, filename = os.path.split(fname1)
        choice = easygui.ccbox(msg='是否打开要查询的文件"%s"' % filename, title='柔板价格核对', choices=('yes', 'no'))
        if choice == True:
            os.startfile(fname1)
        else:
            pass
