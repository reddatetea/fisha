...
def diedai_qushang_qijian(pricedic,pinming,qi_jian):
    while qi_jian != '2020-01' :
        qi_jian = shangQijian(qi_jian)
        pricedic[qi_jian].setdefault(pinming, {0: 0})
        danjias = pricedic[qi_jian][pinming].keys()
        danjia = max(danjias)
        if danjia != 0 :
            break
        else :
...
def main():
    pricedic = byallpricedic.bypriceDic()
    qi_jian = '2020-07'
    pinming = '98g卷筒纸'
    diedai_qushang_qijian(pricedic,pinming,qi_jian)
