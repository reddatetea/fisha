...
（要求查找mobi,azw ,pdf的格式即可），并把创建一个文件（dianziBooklist.txt）
存放所有找到的文件的路径
'''
...
def search_file(start_dir, target):
    ...
    for each_file in os.listdir(os.curdir):      #获取当前工作目录os.curdir下的文件列表！
        ...
        if ext in target :
            dianzibook_list.append(os.getcwd() + os.sep + each_file + os.linesep)  # 使用os.sep是程序更标准
...
f = open(program_dir + os.sep + 'dianzhibookList.txt', 'w',encoding = 'utf-8')

f.writelines(dianzibook_list)
f.close()
