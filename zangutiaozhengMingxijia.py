...
def getShuju():
    ...
    dfe.drop(labels=['品名'], inplace=True, axis=0)
    dfe = dfe.loc[~dfe.index.isnull()]  # 删除索引为空的记录
    # print(dfe)
    fen = dfe.loc[dfe.isnull().sum(1) > 1].index
    gongyingshang = list(fen)[0]
    dfe = dfe.replace('-', '0.0')

    df1 = dfe.loc[:gongyingshang, :]
    df1 = df1.drop(gongyingshang, axis=0)
    cols = dfe.columns.to_list()
    for j in cols:
        df1[j] = df1[j].str.replace(',', '')
    df1 = df1.astype('float64')
    df11 = df1 * (-1)

    df2 = dfe.loc[gongyingshang:, :]
    df22 = df2.drop(gongyingshang, axis=0)
    for j in cols:
        df22[j] = df22[j].str.replace(',', '')
