...
df_dianshang_seji = pd.read_excel(fname_dianshang,sheet_name = '设计费用')

resutl = pd.concat([df_dianshang_yunying,df_dianshang_changku,df_dianshang_seji])
#电商工资,工资表简化
df1 = df[['姓名','实发数']]
#运营人员工资
...
#计计人员工资
seji_gongzi = pd.merge(df_dianshang_seji,df1,how = 'inner',left_on = '姓名',right_on = '姓名')

result = pd.concat([yunying_gongzi,changku_gongzi,seji_gongzi])
...
pivot_seji = getPivotTotal(seji_gongzi,'6602043001','管理费用')

max_row1 = pivot_yunying.shape[0]
max_row2 = pivot_changku.shape[0]
...
with pd.ExcelWriter(newfname,engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
    ...
    pivot_seji.to_excel(writer, sheet_name ='pingzheng', startrow=max_row + 2 + max_row1 + 2 + max_row2  + 2, index=False)
