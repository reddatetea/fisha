...
def Dangyue(start_riqi,end_riqi):
    ...
    os.chdir(path)
    filename = '背条当月入库%s.xlsx'%dtrq
    fname = os.path.join(path, filename)
    fname_gy = r"F:\a00nutstore\006\zw\price\背条供应商.xlsx"
    jianchen = NewGongyinshang.Gongyingshang(fname_gy)
    # jianchen = {'昆山楚宏纸塑材料有限公司':'昆山楚宏',
    #             '苏州市三鑫纸艺':'苏州三鑫'
    #             }
    ...
    for j in ['leibie']:
        df[j] = None
    for j in ['kuan','single_price','合同单价','合同金额', '多计', '多计金额']:
        df[j] = 0.0                             #0.0是flaost64,0是ind64
    df [ '备注'] = None
    df = df.assign(leibie=df['品名'].map(lambda  x: beitiaojisuanPd.beitiaoJisuan(x)[0]))
    df = df.assign(kuan=df['品名'].map(lambda x: beitiaojisuanPd.beitiaoJisuan(x)[1]))
    df = df.reset_index()
    return fname,jianchen,df

def Hetong(end_riqi):
    fname = r"F:\a00nutstore\006\zw\price\背条合同价格.xlsx"
    sheet_name = '合同单价'
    df = pd.read_excel(fname, sheet_name, index_col=0)
    # df['kuan'] = df['kuan'].astype(str)
    grouped = df.groupby('供货单位')
    all_values = []
    for gys, values in grouped:
        grouped2 = values.groupby(['leibie','kuan'])
...
def dfMerge(df1, df2):
    df8 = pd.merge(df1, df2, on=['日期', '供货单位','leibie','kuan'], how='left')  # 连接！从python的传统字典到pandas的连接！量变到质变的飞跃！
    return df8

def huizhong(jianchen,fname,df8):
    df8['single_price'] = df8['danjia']
    # df8['kuan'] = df8['kuan'].astype(float)
    df8['合同单价'] = df8['single_price']*df8['kuan']/1000
