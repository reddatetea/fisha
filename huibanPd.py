...
def  Dangyue(start_riqi,end_riqi):
    ...
    fname_gy = r'F:\a00nutstore\006\zw\price\灰板供应商.xlsx'
    ...
    df = df.assign(单张吨数=df['品名'].map(lambda x: huibanjisuan.huibanchicun(x)[0]))
    df['类别'] = df['品名'].map(lambda x: huibanjisuan.huibanchicun(x)[3])
