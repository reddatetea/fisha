...
def quchu(fname,riqi):
    ...
    if 'DDD' in sheetnames:
        ...
        ws2 = wb[choice]
    wb.close()
    return wb,ws1,ws2,ws11,ws22

def leibiePinmingDic(ws1):
    # 生成品名的当日字典
    max_row1 = ws1.max_row
    leibie_dic = {ws1['A{}'.format(row)].value: (ws1[row][3].value, ws1[row][4].value, ws1[row][5].value) for row in
                  range(2, max_row1 + 1)}
    pinming_dic = {ws1['B{}'.format(row)].value: (ws1[row][3].value, ws1[row][4].value, ws1[row][5].value) for row in
                   range(2, max_row1 + 1)}
    return leibie_dic,pinming_dic

def baobiaoTaitou(ws2,ws3):
    ws01 = ws2['A1:G{}'.format(ws2.max_row)]
    ws02 = []
    for row in ws01:
        hang = []
        for j in row:
            hang.append(j.value)
        ws02.append(hang)
    for  index,row in enumerate(ws02):
        ws3.append(row)
    ws3['D1'].value = '月初'
    ws3['E1'].value = '入库累计'
    ws3['F1'].value = '出库累计'
...
def baobiao(leibie_dic,pinming_dic,ws3):
...
def  main():
    ...
    ws3 = wb3.active
    ws3.title = '原材料日报表'
    wb,ws1,ws2,ws11,ws22 = quchu(fname,riqi)
    fname1 = '原材料日报表{}.xlsx'.format(riqi)
    wb3.save(fname1)
    wb3 = openpyxl.load_workbook(fname1)
    ws3 = wb3['原材料日报表']
    leibie_dic, pinming_dic = leibiePinmingDic(ws1)
    ws3 = baobiaoTaitou(ws2, ws3)
    ws3 = baobiao(leibie_dic,pinming_dic,ws3)
    ws4 = lingjeicun(wb3, ws3)

   #长期库存
    wb3.create_sheet(title = '长期库存表')
    ws33 = wb3['长期库存表']
    leibie_dic1, pinming_dic1 = leibiePinmingDic(ws11)
    ws33 = baobiaoTaitou(ws22, ws33)
    ws33 = baobiao(leibie_dic1, pinming_dic1,ws33)
    wb3.save(fname1)
