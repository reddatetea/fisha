...
需将莱特纸品的利润表和资产负债表以txt的格式导出后，再更名为csv
文件名中必须有“利润”
'''
import pandas as pd
import os
...
def getLaite(fname_laite):
    # fname_laite_xls = r"F:\repos\newfish\莱特利润表2024-12.xls"
    #读取不了中文时的处理方法
    content=xlrd.open_workbook(filename=fname_laite,encoding_override='gbk')
    df_laite=pd.read_excel(content,engine='xlrd')
...
pattern = r'(\d+-\d+).*'
mat = re.search(pattern,path)
if mat:
    qijian = mat.group(1)
else :
    qijian = ''
yue = str(qijian.split('-')[-1])
...
sheet_name =  yue + '费用明细'
filename = qijian + '各公司费用明细汇总.xlsx'
...
with pd.ExcelWriter(filename,engine='openpyxl',mode='a', if_sheet_exists='overlay')  as writer:
    ...
    result.to_excel(writer, sheet_name = f'{yue}汇总',index = False)
