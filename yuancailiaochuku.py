...
def main():
    ...
    filename2 = '原材料' + path.split('\\')[-1] + '出库参考'+'.xlsx'
    ...
    qijian_string = path.split('\\')[-1]                #将'F:\a00nutstore\006\zw\2020\202012'处理为'202012'
    qijian = qijian_string[:4]+'-' +qijian_string[-2:]               #选取文件夹202012作为当月期间，并计算下一期间！
    if int(qijian[-2:]) != 12:
    ...
    #以上形成出库参考
    fname, ws_name = chukucankao.delKongRow(fname2)      #复制出库参考，删除空行
    chukucankao.insertRows(fname, ws_name)               #添加空行，以小计材料各类
    chukucankao.reWriteGongsi(fname, ws_name)            #添加空行后，公式变动了，故重新添加公式！程序结束后直接打开文件
