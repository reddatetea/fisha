...
regex_invoice_number = re.compile(r'[账单编号:|Tax Invoice #] (?P<invoice_number>P\d{9})')
regex_invoice_date = re.compile(r'[账单出具日期:|Invoice Date:]\s+(?P<invoice_date>\d{2}-\w+-\d{4})')
...
for pdf_file in pdf_files:
    ...
    for page in pdf.pages:
        ...
        for line in invoice:
            ...
            if line.startswith('客户编号:') or line.startswith('Customer #:'):
        ...
        for line in invoice:
            if line.endswith('CNY'):
                if line.startswith('总额'):
                    continue
                elif line.startswith('Amount Due'):
                    continue
                elif line.startswith('WUHAN CHINA'):
                    continue
                elif line.startswith('武汉 430050'):
                    continue
                else:
                    item_line += 1
                    item_num += 1
