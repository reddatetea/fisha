...
2024-8-25 调整存货顺序，完全按新软件上的存货分类顺序，还有个bug没解决：1. 用销售订单代替出库时，只能选001库，不能全选
2.新的存货档案不能写进小程序
'''
import os
import datetime
...
def chuli(fname, store_num):
    ...
    df.columns = lst
    # df.columns = lst2
    dic_column = dict(zip(lst, lst2))
    df = df.rename(columns=dic_column)
    if store_num == '001库':
        ...
        df = df.iloc[:, 2:]
        df = df[lst3]
        df = getGroupby(df)           #merge前必须要先汇总，切记
...
def getGroupby(df):
    gp = df.groupby(['class02', 'class05', 'code', 'stock'])
    content = gp['content'].mean()
    begin_ben = gp['begin_ben'].sum()
    begin_jian = gp['begin_jian'].sum()
    ruku_ben = gp['ruku_ben'].sum()
    ruku_jian = gp['ruku_jian'].sum()
    chuku_ben = gp['chuku_ben'].sum()
    chuku_jian = gp['chuku_jian'].sum()
    end_ben = gp['end_ben'].sum()
    end_jian = gp['end_jian'].sum()
...
fname_canchengpin = f'产成品日报表{cangku}{riqi}.xlsx'
fname_yuanbiao = f'工厂日报表{cangku}{riqi}原表.xlsx'
...
if choice:
    ...
    df2_df1 = pd.merge(df2, df1, how='outer', on=['class02', 'class05', 'code', 'stock'], sort=False)
    msg = '请点选工厂昨天的销售订单'
    ...
    # df1 = df1.rename(columns = dict(zip(lst01_yesterday,lst02_yesterday)))
    merge = pd.merge(df2, df1, how='outer', on=['class02', 'class05', 'code', 'stock'], sort=False)  # merge
    merge = merge.fillna(0)
    merge['content'] = (merge.begin_ben + merge.ruku_ben) / (merge.begin_jian + merge.ruku_jian)
...
for name, gp002 in gp:
    ...
    with pd.ExcelWriter(fname_canchengpin, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        ...
        df005 = df005.rename(columns = dict(zip(lst_result0,lst_result)))
