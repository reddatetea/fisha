...
columns= ['开奖期号','开奖日期','开','奖','号','投注总额','单选注数','单选金额','组三注数','组三金额','组六注数','组六金额']
df0 = pd.read_csv(r'http://www.17500.cn/getData/p3.TXT',sep=' ',header=None,names=columns)
...
for i in wumas:
    ...
    for x, y, z in zip(df0['bai'], df0['shi'], df0['ge']):
        if x and i and y in i and z in i:
            data0.append(1)
        else:
            data0.append(0)
    data.append(data0)
