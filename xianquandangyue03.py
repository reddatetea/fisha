...
import os
import datetime
import xianquanjisuanPd03
import xianhuanpricedic
...
def Dangyue(start_riqi,end_riqi):
    ...
    df['备注'] = None
    df = df.assign(规格=df['品名'].map(lambda x: xianquanjisuanPd03.chicun(x)[0]))
    df = df.assign(color=df['品名'].map(lambda x: xianquanjisuanPd03.chicun(x)[1]))
    df = df.assign(齿数=df['品名'].map(lambda x: xianquanjisuanPd03.chicun(x)[2]))
    df = df.reset_index()
    return fname, jianchen, df

def Hetong(end_riqi):
    fname = r'F:\a00nutstore\006\zw\price\线圈合同价格03.xlsx'
    sheet_name = '合同单价'
...
def huizhong(jianchen,fname,df8):
    ...
    df8['单价（元/条)'] = (df8['单价（元/齿）'] * df8['齿数']).where(df8['单位'] == '条', df8['单价（元/齿）'])
    df8['合同金额'] = df8['单价（元/条)'] * df8['入库数量']
