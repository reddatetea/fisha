...
def Dangyue(start_riqi,end_riqi):
    ...
    df['规格'] = None
    df['color'] = None
    for j in ['齿数', '单价（元/齿）','单价（元/条)','合同金额','多计']:
...
def Hetong(end_riqi):
    ...
    for gys, values in grouped:
        grouped2 = values.groupby(['规格','color'])
...
def dfMerge(df1, df2):
    df8 = pd.merge(df1, df2, on=['日期', '供货单位','规格','color'], how='left')  # 连接！从python的传统字典到pandas的连接！量变到质变的飞跃！
    return df8

def huizhong(jianchen,fname,df8):
    df8['单价（元/齿）'] = df8['danjia(元/齿)']
    df8['单价（元/条)'] = round(df8['单价（元/齿）'] * df8['齿数'], 4)
