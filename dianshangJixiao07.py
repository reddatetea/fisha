...
xiangmu_pingtai = {'拼多多-旗舰': '拼多多-旗舰店',
                   '拼多多-专卖': '拼多多-专卖店',
                   '阿里巴巴-1688': '阿里巴巴-1688',
                   '阿里巴巴-分销商': '阿里巴巴-分销商',
                   '淘宝-旗舰-天猫': '天猫-旗舰店',
...
pingtai_xiangmu = {v: k for k, v in xiangmu_pingtai.items()}

mingxi_jiner = {'拼多多-旗舰': 0,
                '拼多多-专卖': 0,
                '阿里巴巴-1688': 0,
                '阿里巴巴-分销商': 0,
                '淘宝-旗舰-天猫': 0,
...
dingdan_jixiao = {'1688-双佳-陈晓': '阿里巴巴-1688',
                  '{线下}': '线下',
                  '分销商': '阿里巴巴-分销商',
                  '抖音-企业店-杨杰': '抖音-企业-直播1',
                  '惊喜-双佳-邵劭': '京东-惊喜',
...
# 先对各项目进行初始赋值 ，如以后有新的则覆盖，如果没有则各项目金额都为初始值0
df0 = pd.DataFrame(columns=['编码', '电商88', '方向2', '期初余额', '本期借方发生', '本期贷方发生', '方向6', '期末余额'])

for i in mingxi_pinyin.values():
    exec(f'{i}' + '=df0')
...
def GetMingxi(k, v):
    ...
    if not v.empty:
        if k != '销售额':
            mingxi = dict(v.set_index('电商88')['本期借方发生'])
        else:
            mingxi = dict(v.set_index('电商88')['本期贷方发生'])
...
for k, v in dfs.items():
    if k != '权益':  # 不处理权益
        ...
        exec(f'{v}' + '=dfs.get(k,v)')
    else:
        pass

moban = pd.read_excel(r"F:\a00nutstore\008\zww08\002电商\电商考核绩效电商销售分析\电商考核表电商销售报表-模板412.xlsx")
moban.set_index('平台名称', inplace=True)
for i in column_pinyin.keys():
    mingxi = GetMingxi(i, eval(column_pinyin.get(i)))
    pingtai1 = moban.index.map(pingtai_xiangmu)
    moban[f'{i}'] = pingtai1.map(mingxi)
...
def tiaozhengGongzi(d, s):
    s1 = s.copy()
    s1 = s1.fillna(0)
    s2 = pd.Series({'拼多多-旗舰': 1500, '阿里巴巴-1688': 1500, '抖音-直播1': 2000})
    s3 = s1 + s2
    s3 = s3.fillna(0)
    d['工资'] = s3
...
moban['毛利率'] = moban['毛利率'].fillna(0).map(lambda x: f'{x:.2%}')
moban['净利率'] = moban['净利率'].fillna(0).map(lambda x: f'{x:.2%}')

res = f'电商考核表电商销售报表{qijian}.xlsx'
moban.to_excel(res)
os.startfile(res)
