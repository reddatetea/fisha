...
def totalPancunbiao():
    ...
    lst = ['存货大类名称', '存货名称', '计量单位', '期初结存数量', '总计_入库数量', '总计_出库数量', '期末结存数量', '生产领料_出库数量']
    df = df[lst]
    df.columns = ['存货大类名称', '品名', '单位', '上日数量', '入库数量', '出库数量', '本日数量', '生产领料_出库数量']
    df1 = df.assign(cwName=df['品名'].map(lambda x: singlecailiaoName(x)[0]))
    # 这里要加两条语句，规避品名中“箱”，但不属于纸箱、以及纸箱大类中名称里面没有“箱”的两种情形
    #df1 = df1.assign(cwName = np.where((df1.存货大类名称.str.endswith('纸箱')) & (df1.cwName == '纸箱'),'纸箱',df1.cwName))
    df1 = df1.assign(cwName=np.where(df1.存货大类名称.str.endswith('纸箱'), '纸箱', df1.cwName))
    df1 = df1.assign(cwName=np.where((df1.存货大类名称.str[-2:] != '纸箱') & (df1.cwName == '纸箱'), df1.品名, df1.cwName))

    df1.cwName = np.where((df1.cwName == '线圈') & (df1['单位'] == '卷'), '线圈（卷）', df1.cwName)
    df1.cwName = np.where((df1.cwName == '线圈') & (df1['单位'] == '公斤'), '线圈（公斤）', df1.cwName)
    df1 = pd.pivot_table(df1, index='cwName', aggfunc='sum', fill_value=0,
...
def main():
    ...
    fname = r"F:\a00nutsrore\006\zw\baiyun\2020白云入库.xlsx"
    sheet_name = r'2020'
    ...
    fnaem = r"F:\a00nutsrore\006\zw\else\2020入库.xlsx"
    sheet_name = r'入库'
    df = weida(qijian, xia_qijian, fname, sheet_name)
    if not df.empty:
        df['未达项'] = 0
    ...
    if not weida_data.empty:
        ...
        # weida.columns = ['cwName','未达项']
        # weida_pivot = pd.pivot_table(weida_data, index='cwName', aggfunc='sum')
        df2 = df1.assign(未达项 = 0)  # 原表
        df_cankao = df2
    ...
    chuku = []
    jiner = []
    print(df_cankao['cwName'].to_list())
